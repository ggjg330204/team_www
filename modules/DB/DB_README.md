...
안녕하세요! 이 문서는 **데이터(DB)**와 **스토리지(저장소)**를 담당한 엔지니어가 **프로젝트 매니저(PM)**를 위해 작성했습니다.
...
PM님은 이 자판기를 **PM님의 `main.tf` 파일**에 가져다 놓고, **동전(변수 값)**만 넣으시면 됩니다.

- **자판기 위치**: `./modules/DB/database`, `./modules/DB/storage`
- **PM님이 할 일**: 아래 코드를 `main.tf`에 복사하고, 빈칸만 채우기!
...
module "database" {
  ...
  # [2] 리소스 그룹 & 위치 (PM님이 만든 것과 똑같이 맞추세요)
