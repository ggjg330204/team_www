...
- **상태 (Status)**: WordPress 객체 캐싱 활성화
- **통합 세부사항**:
  - PHP Redis 확장 (`php-pecl-redis5`) 설치됨
  - WordPress Redis Object Cache 플러그인 설치됨
  - `wp-config.php`에 Redis 연결 설정 자동 구성
  - SSL 포트 (6380) 사용, Primary Key를 통한 인증
  - **성능 향상**: 응답 시간 2-5배 개선 (캐시 히트율 70%+ 예상)

### 1.4. Data Factory (데이터 팩토리) - `modules/database`
- **이름 (Name)**: `www-adf-{random}`
- **목적**: MySQL 데이터베이스 자동 백업
- **파이프라인 (Pipeline)**: `MySQLDailyBackup`
  - **소스**: MySQL Flexible Server (information_schema)
  - **싱크**: Azure Blob Storage (`mysql-backups` 컨테이너)
  - **형식**: CSV (UTF-8)
  - **스케줄**: 매일 오전 2시 (KST) 자동 실행
- **Managed Identity**: SystemAssigned (보안 액세스)
- **백업 보존**: Blob Storage의 Lifecycle Policy로 관리

### 1.5. CDN - `modules/storage`
- **프로필 (Profile)**: `www-cdn-profile` (Azure Front Door Standard)
- **엔드포인트 (Endpoint)**: `www-cdn-endpoint-{random}`
- **Origin**: Storage Account Blob 연결 완료
- **용도**: 정적 콘텐츠 (이미지, CSS, JS) 전송 가속
...
- **MySQL**:
  - **자동 백업 (Automated Backup)**: 보존 기간 (Retention Period) 35일 (최대 설정)
  - **백업 중복성 (Backup Redundancy)**: Local (기본값) 또는 Geo-Redundant (설정 가능)
  - **Data Factory 백업**:
    - MySQL 데이터를 Blob Storage로 자동 백업
    - 매일 오전 2시 (KST) 실행
    - CSV 형식으로 `mysql-backups` 컨테이너에 저장
    - 재해 복구 및 데이터 보호 강화
- **Storage**:
  - **소프트 삭제 (Soft Delete)**: 7일 (실수로 삭제된 데이터 복구 가능)
  - **버전 관리 (Versioning)**: 활성화됨 (이전 버전 데이터 복구 가능)
  - **Lifecycle Management**: Blob 데이터 자동 계층화 (Hot → Cool → Archive)
...
| **Database** | `redis_ssl_port` | Redis SSL 포트 (6380) | 앱 서버 Redis 연결 |
| **Database** | `redis_primary_key` | Redis 액세스 키 (민감 정보) | 앱 서버 캐시 인증 |
| **Database** | `adf_name` | Data Factory 이름 | 백업 파이프라인 관리 |
| **Storage** | `storage_account_name` | 스토리지 계정 이름 | 앱 서버 파일 저장소 설정 |
| **Storage** | `primary_access_key` | 스토리지 키 (민감 정보) | 앱 서버 스토리지 인증 |
| **Storage** | `storage_connection_string` | 연결 문자열 (민감 정보) | Data Factory Blob 연결 |
