...
### ⚠️ 미완성 / 제약 사항
- **Identity**: Entra ID / RBAC 미구성 (사용자 권한 `Contributor`로 인한 제한)
- **MySQL Replica**: 타 리전(Korea South) 배포 불가로 **동일 리전(Korea Central)**에 구성함

### 🎉 최근 개선 사항
- ✅ **Security 모듈**: NSG 규칙(SSH, HTTP, DB) 적용 완료
- ✅ **CDN**: Standard Profile 배포 실패 후 **Front Door Profile**로 변경하여 배포 성공
  - **성공 이유**: 학생/무료 구독에서는 레거시 `azurerm_cdn_profile` (Standard_Microsoft) 생성이 제한되는 경우가 많으나, 최신 `azurerm_cdn_frontdoor_profile`은 배포가 허용됨.
- ✅ **Redis Cache**: 활성화 완료
...
1. **Read Replica 활성화 (동일 리전)**:
   - **현재 적용됨**: `var.replica_loca` 대신 `var.loca`를 사용하여 Korea Central에 복제본 생성.
   - **이유**: 학생/무료 구독에서 타 리전 리소스 생성 제한이 있음.
...
**평가**: ⚠️ **구현 시도했으나 권한 문제로 실패**

**실패 사유**:
- 현재 사용자의 구독 권한이 **`Contributor` (기여자)**입니다.
- Entra ID 사용자 생성(`azuread_user`) 및 RBAC 역할 할당(`azurerm_role_assignment`)을 수행하려면 **`Owner` (소유자)** 또는 **`User Access Administrator`** 권한이 필수입니다.
- 따라서 코드는 작성되었으나 실제 배포 시 `AuthorizationFailed` 오류가 발생하여 주석 처리하였습니다.
...
**상태**: `modules/Security` 디렉토리에 `01_nsg.tf`, `02_nsg_rules.tf` 파일 존재하며 **구현 완료됨**.
...
|----------|---------|------|----------|
| 인프라 구성 | VNet, VM, Storage 완료 | 90% | MySQL Replica 완료 (동일 리전) |
| 데이터 복원 | GRS 복제 완료 | 100% | ✅ 완료 |
| 네트워킹 | VNet, Subnet 구성 | 90% | ✅ 완료 |
| 보안 | NSG, Private Endpoint 구성 | 80% | RBAC은 권한 문제로 제외 |
| 신원 관리 | 미구성 | 0% | Contributor 권한 한계로 구현 불가 |
| 규정 준수 | 미구성 | 0% | Azure Policy 적용 |

**종합 점수**: **75/100** (보안 및 Replica 구현으로 상승)

### ✅ 개선 완료 항목

- ✅ Storage Account: LRS → GRS 변경 완료
- ✅ Redis, CDN, RBAC 파일 비활성화 (`.disabled`) 처리 완료

### 🎯 다음 단계 체크리스트

```markdown
## 즉시 구현 필요
- [x] NSG 생성 및 Subnet 연결 (완료)
- [ ] Entra ID User 생성 (권한 부족으로 불가)
- [ ] RBAC 역할 할당 (권한 부족으로 불가)

## 권장 구현
> 상세 확장 가이드는 **[Module_Scalability.md](./Module_Scalability.md)**를 참조하세요.
