...
flowchart TD
    ...
    %% --- 노드 정의 ---
    subgraph Root_Layer ["🕹️ 루트 컨텍스트 (Root)"]
        Main("메인 실행 (main.tf)"):::root
    end

    subgraph Core_Layer ["🌐 핵심 인프라 (Core)"]
        direction TB
        N_Core[("가상 네트워크<br/>(VNet / Subnet)")]:::net
        N_Out[["출력값<br/>(outputs.tf)"]]:::net
        N_Core -.-> N_Out
    end

    subgraph Resource_Layer ["📦 애플리케이션 리소스"]
        direction LR
        S_NSG["🛡️ 보안 그룹<br/>(NSG)"]:::security
        C_VM["⚙️ 컴퓨팅<br/>(VM / VMSS)"]:::compute
        DB_Res[("🛢️ 데이터베이스<br/>(SQL / Redis)")]:::data
        ST_Res[("💾 스토리지<br/>(Storage)")]:::data
    end
    ...
    %% 의존성 흐름
    N_Out -.-> |서브넷 ID| S_NSG
    N_Out ==> |서브넷/NIC 연결| C_VM
    N_Out -.-> |VNet 연동| DB_Res
    N_Out -.-> |서비스 엔드포인트| ST_Res

    %% 논리적 연결
    ST_Res -.-> |연결 문자열| DB_Res
```
...
flowchart TB
    ...
    %% Azure 클라우드 영역
    subgraph Cloud ["☁️ Microsoft Azure 클라우드"]
        direction TB

        %% 트래픽 진입
        subgraph Ingress ["🚦 트래픽 진입점"]
            LB["⚖️ 로드 밸런서"]:::lb
        end

        %% 리전 1: 한국 중부
        subgraph KR_Central ["🇰🇷 한국 중부 (메인 리전)"]
            direction TB
            
            subgraph Mgmt_Layer ["관리 영역 (Mgmt)"]
                Bastion["🛡️ 배스천 호스트"]:::bastion
            end

            subgraph Web_Layer ["웹 계층 (Web Tier)"]
                WebVM1["🖥️ 웹 서버 1"]:::web
                WebVM2["🖥️ 웹 서버 2"]:::web
            end
            
            subgraph Data_Layer_1 ["데이터 계층 (Data Tier)"]
                MySQL_Pri[("🛢️ MySQL (메인)")]:::db
                Storage["📦 스토리지 계정"]:::db
            end
        end

        %% 리전 2: 한국 남부
        subgraph KR_South ["🇰🇷 한국 남부 (재해복구/확장)"]
            direction TB
            
            subgraph App_Layer ["앱 계층 (App Tier)"]
                VMSS["⚙️ VM 스케일 셋"]:::app
            end

            subgraph Data_Layer_2 ["데이터 복제 (Replica)"]
                MySQL_Rep[("🛢️ MySQL (복제본)")]:::db
            end
        end
    end
    ...
    Bastion -.-> |관리 접속| WebVM1 & WebVM2 & VMSS

    WebVM1 & WebVM2 --> |쿼리| MySQL_Pri
    VMSS --> |로직 처리| MySQL_Pri

    MySQL_Pri -.-> |데이터 동기화| MySQL_Rep
