...
resource "azurerm_linux_virtual_machine" "www_basvm" {
  ...
  size                  = "Standard_B1s"
  admin_username        = "www"
  network_interface_ids = [var.nic_bas_id]
  ...
  user_data = base64encode("${file("${path.module}/scripts/00_sh_key.sh")}${file("${path.module}/scripts/00_sh_bas.sh")}")
...
resource "azurerm_linux_virtual_machine" "www_web1vm" {
  ...
  size                  = "Standard_B1s"
  admin_username        = "www"
  network_interface_ids = [var.nic_web1_id]
  ...
  user_data = base64encode(file("${path.module}/scripts/00_sh_web.sh"))
...
resource "azurerm_linux_virtual_machine" "www_web2vm" {
  ...
  admin_username        = "www"
  network_interface_ids = [var.nic_web2_id]
...
resource "azurerm_linux_virtual_machine" "www_dbvm" {
  ...
  user_data = base64encode(file("${path.module}/scripts/00_sh_db.sh"))
...
resource "azurerm_linux_virtual_machine" "www_vnet1_web1vm" {
  ...
  size                  = "Standard_B4as_v2"
  admin_username        = "www"
  network_interface_ids = [var.nic_web1_v1_id]
  admin_ssh_key {
    username = "www"
    #public_key = file("${path.module}/../../ssh/id_ed25519.pub")
    public_key = file("${path.module}/keys/id_ed25519.pub")
