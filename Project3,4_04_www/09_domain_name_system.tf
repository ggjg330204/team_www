...
resource "azurerm_dns_cname_record" "www_public" {
  ...
  resource_group_name = azurerm_resource_group.rg.name
  ttl                 = 300
  record              = module.storage.frontdoor_endpoint_hostname
}

# DNS validation for Front Door custom domain (www.04www.cloud)
...
# Root domain A record (points to Load Balancer)
