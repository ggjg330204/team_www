resource "azurerm_linux_virtual_machine_scale_set" "was_vmss" {
  ...
  extension {
    ...
    protected_settings = jsonencode({
      script = base64encode(templatefile("${path.module}/../../scripts/was_init.tftpl", {
        db_host          = var.db_host
        ...
        mail_server_ip   = azurerm_linux_virtual_machine.mail_vm.private_ip_address
...
resource "azurerm_virtual_machine_scale_set_extension" "was_ama" {
  name                         = "AzureMonitorLinuxAgent"
  virtual_machine_scale_set_id = azurerm_linux_virtual_machine_scale_set.was_vmss.id
