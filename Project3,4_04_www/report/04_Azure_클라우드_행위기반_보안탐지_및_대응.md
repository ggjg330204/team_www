...
> [!NOTE] 스크린샷 가이드: SSH Brute Force 탐지
> *   **Image 1 (공격):** Kali Linux 터미널에서 `hydra` 공격이 진행되며 `[ERROR] target ssh://...` 메시지가 쏟아지는 화면.
> *   **Image 2 (탐지):**
>     1.  **Microsoft Sentinel > Incidents** 메뉴 클릭.
>     2.  **심각도**가 빨간색(**High**)으로 표시된 **'SSH Brute Force Attack'** 인시던트가 목록 최상단에 있는 화면.
