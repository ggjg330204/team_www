...
| **Storage** | `azurerm_storage_account` | 3 | 메인(Blob), Premium(File), 함수앱용 |
| | `azurerm_storage_container` | 5+ | 미디어, 로그, 백업, 상태 저장소 등 |
...
5.  **처리 (Processing)**: WAS는 이미지를 임시 처리하고, **Private Endpoint**를 통해 **Storage Account (Blob)**의 컨테이너에 원본을 저장합니다. (인터넷 경유 X, 내부망 전송)
...
- **Security Configuration**:
    ...
    - **Access Control**: SSH(22) 접근은 `ssh_allowed_ips` 변수에 지정된 관리자 IP로만 제한.
...
12. **Mass HTTP Requests**: 단일 IP의 대량 요청(DDoS/Crawling) 탐지
...
**[SOAR 자동화 알림]**
- **Action Group**: `sentinel-alert-email` - 인시던트 발생 시 이메일 알림 발송
- **Automation Rule**: 인시던트 생성 시 상태를 자동으로 'Active'로 전환
...
- **Access Policy**: Managed Identity(AppGw, VMSS)에 대해 RBAC 최소 권한 할당
...
**[주요 보안 정책]**
- **Mail Server SSH**: 공인 IP를 보유하고 있으나, NSG를 통해 지정된 관리자 IP 대역에서만 SSH 접속을 허용합니다 (인터넷 전체 개방 차단).
