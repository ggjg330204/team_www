...
총 **294개 이상**의 Azure 리소스 인스턴스가 유기적으로 연결되어 대규모 엔터프라이즈 서비스를 구성하고 있습니다. (Terraform State 기준)
...
- **주요 정책**:
    ...
    - `Allow-MySQL`: TCP 3306 포트 허용 (MySQL Flexible Server 연결)
    - `Allow-Redis`: TCP 6379, 6380 포트 허용 (Redis Cache 연결)
...
- **Direct Connection (성능 최적화)**: VMSS의 Private IP(NIC)를 Backend Pool에 **직접 등록**하여, 불필요한 Public Load Balancer 경유를 제거하고 리전 내부(Private Network)에서 트래픽을 처리합니다.
- **Routing Optimization (비대칭 라우팅 방지)**: Application Gateway Subnet은 비대칭 라우팅 문제를 방지하기 위해 **User Defined Route (UDR)를 적용하지 않고**, Azure VNet의 기본 시스템 라우팅을 따릅니다. 이를 통해 인바운드 및 아웃바운드 트래픽 경로의 대칭성을 보장합니다.
...
**[기술적 도입 배경 및 의의]**
- **Outbound NAT**: VMSS 인스턴스들이 외부 인터넷과 통신할 때 사용하는 공인 IP(SNAT)를 제공합니다.
- **L4 부하분산**: (필요시) TCP/UDP 레벨의 트래픽 분산을 담당합니다. 
*참고: 웹 트래픽(HTTP/S)은 App Gateway가 직접 처리하므로, Load Balancer는 주로 아웃바운드 통신 및 SSH 접속(Inbound NAT) 용도로 사용됩니다.*
...
- **운영 현황 (As-Is)**: 현재 Redis는 데이터 캐싱 용도로 주로 활용되고 있으며, PHP Session 저장은 서비스 안정성을 최우선으로 고려하여 **File System** 방식을 유지하고 있습니다. (추후 Redis 전환 검토)
