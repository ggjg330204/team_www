...
- **Custom Domain** (중요 업데이트):
    - **도메인**: `www.04www.cloud`, `04www.cloud`
    - **TLS 인증서**: `ManagedCertificate` (Azure Front Door가 자동 발급 및 갱신)
    - **DNS 검증**: `_dnsauth.www` TXT 레코드를 통해 도메인 소유권 검증
    - *근거*: Let's Encrypt과 달리 Front Door Managed Certificate는 Azure가 자동으로 발급 및 갱신하며, ACME Provider 설정이 불필요하여 운영 부담이 적습니다.

- **Origin Group**:
    - **Health Probe**: `HTTP / HEAD / /health.html`
    - **Interval**: `100초`
    - *근거*: WAS VMSS 백엔드의 health.html 파일을 주기적으로 확인하여, 장애 발생 시 자동으로 트래픽을 차단합니다.

- **Origin** (변경됨 - 중요):
    - **Host**: **Load Balancer Public IP** (이전: Storage Account Blob Host)
    - **Priority**: `1`, **Weight**: `1000`
    - **Certificate Name Check**: `false` (IP 주소 사용 시 필수)
    - *근거*: 원래 정적 콘텐츠(Storage Account)를 서비스하려던 계획에서, **동적 PHP 애플리케이션 (WAS VMSS)**을 서비스하도록 변경되었습니다. 이를 통해 SQL Injection, XSS 등 보안 취약점 실습 랩을 Front Door WAF로 보호할 수 있습니다.

- **Route**:
    - **Patterns**: `/*` (모든 경로 매칭)
    - **Forwarding Protocol**: `MatchRequest` (HTTP는 HTTP로, HTTPS는 HTTPS로 전달)
    - **HTTPS Redirect**: `Enabled` (보안 통신 강제)
    - **Custom Domains**: `www_custom_domain`, `apex_custom_domain` 연결
    - *근거*: 사용자가 `www.04www.cloud`로 접속하면 Front Door WAF를 경유하여 WAS VMSS로 전달되며, WAF 탐지 여부가 홈페이지에 표시됩니다.

**[DNS 설정]**
- **CNAME 레코드**: `www.04www.cloud` → Front Door Endpoint Hostname
- **TXT 레코드**: `_dnsauth.www.04www.cloud` → 도메인 검증 토큰
- **A 레코드**: `04www.cloud` (루트) → Load Balancer Public IP (CNAME 사용 불가)
    - *참고*: Root 도메인은 DNS 표준상 CNAME을 사용할 수 없으므로 A 레코드로 직접 Load Balancer를 가리킵니다.
