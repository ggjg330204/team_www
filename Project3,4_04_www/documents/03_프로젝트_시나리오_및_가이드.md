...
*   **참고:** Terraform `modules/Compute/01_web_vmss.tf` & `modules/Compute/05_autoscale_setting.tf`
...
*   **참고:** Terraform `modules/Serverless/02_function.tf`
...
*   **참고:** Terraform `modules/ContainerRegistry/01_azure_container_registry.tf`
...
*   **참고:** Terraform `modules/Compute/03_mail_vm.tf`
...
*   **현재 역할 구성:**
    | 계정 | 역할 | Azure Role | Scope |
    |------|------|------------|-------|
    | `student421` | PM | Owner | Resource Group |
    | `student424` | 아키텍처검증 | Reader | Resource Group |
    | `student420` | 내부 보안 | Security Admin | Resource Group |
    | `student415` | 내부 보안 | Log Analytics Reader | Log Analytics |
    | `student411` | 외부 보안 | Sentinel Contributor | Log Analytics |
    | `student426` | 외부 보안 | Sentinel Contributor | Log Analytics |
*   **검증 방법:**
    1.  `student424@mscsschool.onmicrosoft.com` (Reader)로 로그인.
    2.  리소스 생성 시도 → **'권한 없음(AuthorizationFailed)'** 실패 확인.
...
**12. 역할별 권한 범위(Scope) 검증**
*   **담당자:** 기훈
*   **내용:** 각 역할이 할당된 Scope 내에서만 권한을 행사할 수 있는지 검증합니다.
*   **시나리오 A (Security Admin - student420):**
    1.  `student420@mscsschool.onmicrosoft.com`으로 Azure Portal 로그인.
    2.  **보안 설정 확인 (성공)**: Defender for Cloud → 보안 정책 조회 가능.
    3.  **리소스 삭제 (실패)**: VM 또는 DB 삭제 시도 → **'권한 없음'** 확인.
*   **시나리오 B (Log Analytics Reader - student415):**
    1.  `student415@mscsschool.onmicrosoft.com`으로 Azure Portal 로그인.
    2.  **로그 조회 (성공)**: Log Analytics Workspace → 쿼리 실행 가능.
    3.  **설정 변경 (실패)**: 로그 보존 기간 변경 시도 → **'권한 없음'** 확인.
*   **시나리오 C (Sentinel Contributor - student411):**
    1.  `student411@mscsschool.onmicrosoft.com`으로 Azure Portal 로그인.
    2.  **Sentinel 규칙 관리 (성공)**: Sentinel → 분석 규칙 생성/수정 가능.
    3.  **타 리소스 접근 (실패)**: VM, DB 등 Sentinel 외 리소스 수정 시도 → **'권한 없음'** 확인.
*   **참고:** Terraform `modules/Identity/02_role_assignments.tf`
...
*   **참고:** **[제한사항]** Entra ID, Office 365 등 커넥터는 전역 관리자 권한 필요.
...
**8. SOAR 실시간 대응 (Action Group 알림)**
*   **담당자:** 현지
*   **내용:** 보안 사고 자동 알림을 구현합니다.
*   **방법:** Sentinel 인시던트 발생 시 Action Group 트리거 -> 이메일 알림 수신 확인.
*   **참고:** Terraform `modules/Security/09_sentinel_playbooks.tf` (Action Group + Alert Rule)
...
**22. SQL Injection 공격 시도 탐지 및 알림**
*   **담당자:** 현지
*   **내용:** 공격 탐지 및 운영자 알림을 검증합니다.
*   **방법:** SQL Injection 시도 -> Sentinel 탐지 -> Action Group 이메일 알림 발송.
*   **참고:** Sentinel Alert Rule (SQL Injection)
