...
Logic App이 Sentinel 경고를 받아 메일을 보내려면 **"Outlook.com"** 커넥터를 연결해야 합니다.

#### **A. Logic App 연결 (Outlook.com) - 메일용**
*   **경로**: Logic App > API Connections > `Outlook.com` (디자이너에서 추가)
*   **작업**:
    1.  **"Outlook.com"** 커넥터를 검색해서 추가하세요.
    2.  본인의 **개인 MS 계정** (hotmail, outlook.com 등)으로 로그인하면 바로 됩니다.
...
### 2.2 Web 서버 접속 후 변수 설정 (서버 내부 Bash)

**⚠️ 중요:** Web 서버에 SSH 접속한 후에는 로컬 PC의 PowerShell 변수가 없습니다!  
아래 명령어를 Web 서버 **Bash 셸**에서 실행하여 변수를 다시 설정하세요.

```bash
# Web 서버(Bash) 내부에서 실행
export WAS_IP="192.168.5.6"
export MYSQL_HOST="www-mysql-server-twfs.mysql.database.azure.com"
export REDIS_HOST="www-redis-cache-aejd.redis.cache.windows.net"
...
> **💡 Tip:** 위 값들은 `terraform output` 명령어로 로컬에서 먼저 확인한 후, 복사해서 사용하세요.

---

### 2.3 WAS VMSS 접속 (Private IP) - 3가지 방법
...
1. **터널 생성 (터미널 1)** - 이 창은 켜두세요.
   ...
   ssh -N -L 2222:192.168.5.6:22 -p $SSH_PORT www@$WEB_PUBLIC_IP
   ```
...
```powershell
ssh -o ProxyCommand="ssh -p $SSH_PORT -W %h:%p www@$WEB_PUBLIC_IP" www@192.168.5.6
```
...
```bash
# Web 서버 Bash에서 실행
mysql -h www-mysql-server-twfs.mysql.database.azure.com -u www -p
# 비밀번호: It12345!
```
...
1. **터널 생성 (터미널 1):** 로컬 3307 포트를 MySQL 3306 포트로 연결
   ```powershell
   ssh -N -L 3307:www-mysql-server-twfs.mysql.database.azure.com:3306 -p $SSH_PORT www@$WEB_PUBLIC_IP
   ```
2. **DB 툴 접속 정보:**
...
**방법 B: 로컬 터널링**
```powershell
ssh -N -L 6380:www-redis-cache-aejd.redis.cache.windows.net:6380 -p $SSH_PORT www@$WEB_PUBLIC_IP
```
*터널링 후 로컬에서 `redis-cli -h 127.0.0.1 -p 6380 -a ... --tls` 접속*
...
# 3. Application Gateway 직접 접속 테스트
$APPGW_IP = az network public-ip show --resource-group $RG_NAME --name "www-appgw-pip" --query ipAddress -o tsv
...
| **WAS 터널링** | `ssh -N -L 2222:192.168.5.6:22 -p $SSH_PORT www@$WEB_PUBLIC_IP` |
| **MySQL 터널링** | `ssh -N -L 3307:$MYSQL_HOST:3306 -p $SSH_PORT www@$WEB_PUBLIC_IP` |
| **Redis 터널링** | `ssh -N -L 6380:$REDIS_HOST:6380 -p $SSH_PORT www@$WEB_PUBLIC_IP` |
...
| 작업 | 명령어 |
|------|--------|
| **WAS 접속** | `ssh www@192.168.5.6` |
| **MySQL 접속** | `mysql -h $MYSQL_HOST -u www -p` |
| **Redis 접속** | `redis-cli -h $REDIS_HOST -p 6380 -a '<KEY>' --tls` |
